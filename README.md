...
Visual Studio 없이 설치하려면 [최신 GitHub Release](https://github.com/Leuconoe/KeyOveray/releases/latest)에서 x64 ZIP 파일을 다운로드합니다.

> Microsoft Defender 탐지에 대한 오탐 검토가 진행 중인 프리릴리스입니다. Microsoft 판정 전에는 개인 테스트 용도로만 사용하세요. 설치기는 기존 패키지를 먼저 제거하므로 저장된 키 배열이 초기화될 수 있습니다.
...
```powershell
Get-ChildItem .\KeyOverlay-*-x64.zip | Get-FileHash -Algorithm SHA256
```
